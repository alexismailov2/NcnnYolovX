...
public:
  TimeMeasuring(OnFinishedCallback &&onFinished) noexcept
    : _onFinished{std::move(onFinished)}, _startTime{std::chrono::steady_clock::now()}
  {
  }
...
#define TAKEN_TIME() \
   ...
   auto __$$$__tm = OIYolo::TimeMeasuring<::std::chrono::microseconds>([__$$$__fn](uint64_t takenTime) { \
